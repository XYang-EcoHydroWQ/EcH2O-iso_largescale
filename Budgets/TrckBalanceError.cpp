...
void Budget::TrckBalanceError(const Control *ctrl)
{
	...
	Sold = initsnowpack + initponding + initL1 + initL2 + initL3 + initGW + initchan; //puls channelstorage
	
	outputs = evaporationS + evaporationI + transpiration + ovlndflow + gwtrflow + leakage;
	
	Snew = canopy + snowpack + ponding + soilL1 + soilL2 + soilL3 + grndwater + chan_store;
	...
	if(ctrl->sw_2H){
	  ...
	  Sold_d2H = initsnowpack_d2H + initponding_d2H + initchan_d2H + 
	  ...
	  Snew_d2H = canopy_d2H + snowpack_d2H + ponding_d2H + soilL1_d2H + soilL2_d2H + soilL3_d2H + chan_d2H + 
	    grndwater_d2H;
	...
	if(ctrl->sw_18O){
	  ...
	  Sold_d18O = initsnowpack_d18O + initponding_d18O + initchan_d18O +
