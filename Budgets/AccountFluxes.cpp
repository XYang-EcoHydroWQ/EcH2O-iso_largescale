...
double Budget::AccountFluxes(const grid *map1, const grid *map2, const Atmosphere *a) {
...
#pragma omp parallel for			\
  ...
  for (UINT4 i = 0; i < zones; i++)
    for (UINT4 j = 0; j < a->getSortedGrid()[i].cells.size(); j++) {
      ...
      result += (map1->matrix[r][c] * dx * dx * dt * map2->matrix[r][c]); //map2 gives the ttarea
