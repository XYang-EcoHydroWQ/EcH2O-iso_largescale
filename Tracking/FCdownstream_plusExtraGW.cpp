...
void Tracking::FCdownstream_plusExtraGW(Basin &bsn, Control &ctrl,
			    double &Qk1, double &dtdx, double &dx, int r, int c, int rr, int cc)
{
  ...
  if(ctrl.sw_2H){
	...
	//to update fluxes of current time step
	_Fd2HLattoExtraGW->matrix[rr][cc] += bsn.getFluxExtraGWtoLat()->matrix[r][c] * 
	                                       _d2HExtraGWtoLat->matrix[r][c];
  }
  // Oxygen 18
  if(ctrl.sw_18O){
	...
	_Fd18OLattoExtraGW->matrix[rr][cc] += bsn.getFluxExtraGWtoLat()->matrix[r][c] * 
	                                       _d18OExtraGWtoLat->matrix[r][c];
  }
  ...
  if(ctrl.sw_Age){
	...
	_FAgeLattoExtraGW->matrix[rr][cc] += bsn.getFluxExtraGWtoLat()->matrix[r][c] * 
	                                       _AgeExtraGWtoLat->matrix[r][c];
  }
