 ...
 *******************************************************************************/
/*
 * IncrementAge_plusExtraGW.cpp
 *
 *  Created on: May 14, 2020
 *      Author: Xiaoqiang Yang
 *  Following ./Tracking/IncrementAge.cpp    
 */

#include "Tracking.h"

int Tracking::IncrementAge_plusExtraGW(Basin &bsn, Control &ctrl){
...
#pragma omp for
    for (UINT4 i = 0; i < _AgeOvlndOutput.cells.size(); i++){ //b->getSortedGrid().cells.size();; i++){
	  ...
	  _AgeExtraGWtrOutput.cells[i].val += dt;
      }
