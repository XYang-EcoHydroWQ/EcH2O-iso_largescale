...
void Tracking::OutletVals_plusExtraGW(Basin &bsn, Control &ctrl, int mode, 
                   double Qk1, double Outletpond, int r, int c)
{
  ...
  if (mode == 0){ // Initialization
    ...
    if(ctrl.sw_2H){
	  ...
	  _d2HExtraGWtrOutput.cells.push_back(cell(r, c, _d2HExtraGWtoLat->matrix[r][c])); 
    }
    // Oxygen 18
    if(ctrl.sw_18O){
      ...
      _d18OExtraGWtrOutput.cells.push_back(cell(r, c, _d18OExtraGWtoLat->matrix[r][c]));	  
    }
    // Age
    if(ctrl.sw_Age){
      ...
      _AgeExtraGWtrOutput.cells.push_back(cell(r, c, _AgeExtraGWtoLat->matrix[r][c]));	  
    }
  }
}
