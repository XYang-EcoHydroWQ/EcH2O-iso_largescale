...
int Basin::SolveChannelEnergyBalance(Atmosphere &atm,
  ...
  REAL8 Td = _Temp_d->matrix[r][c]; // temperature of the groundwater; also used for extraGW -- yangx 2021
  ...
  REAL8 SrftoChn = _FluxSrftoChn->matrix[r][c];
  REAL8 ExGWtoChn = 0.0;
  ...
  if (ctrl.sw_extraGW){
    ExGWtoChn = _FluxExtraGWtoChn->matrix[r][c];
  }
  qin = GWtoChn + LattoChn + SrftoChn + ExGWtoChn + chan_prec;
  Tin = qin < RNDOFFERR ? Ts : ((GWtoChn + ExGWtoChn)*Td + T_LattoChn + SrftoChn * Ts + chan_prec * Ta) / qin;
