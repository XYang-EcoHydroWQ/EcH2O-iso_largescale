...
int Report2nc(){
  ...
  if (oControl->Rep_BedRock_leakage)
	oReport->UpdateOutputNC(oBasin->getBedrockLeakage(), "LeakF", "W");	
  ...
  if(oControl->sw_trck && oControl->sw_2H){
    ...
    oReport->UpdateOutputNC(oTracking->getd2HExtraGWtoLat(), "dHExGWO", "TD");
    }
  }
  ...
  if(oControl->sw_trck && oControl->sw_18O){
    ...
    oReport->UpdateOutputNC(oTracking->getd18Oleakage(), "dOLeak", "TO");
      if (oControl->Rep_d18OExtraGWtoLat)
    oReport->UpdateOutputNC(oTracking->getd18OExtraGWtoLat(), "dOExGWO", "TO");
    }
  }
  if(oControl->sw_trck && oControl->sw_Age){
    ...
    oReport->UpdateOutputNC(oTracking->getAgeleakage(), "AgLeak", "TA");
      if (oControl->Rep_AgeExtraGWtoLat)
    oReport->UpdateOutputNC(oTracking->getAgeExtraGWtoLat(), "AgExGWO", "TA");
      if (oControl->Rep_AgeExtraGWtoChn)
    oReport->UpdateOutputNC(oTracking->getAgeExtraGWtoChn(), "AgExOC", "TA");
    }
  }
